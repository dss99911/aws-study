...
- [hello-world](hello-world) : API type의 샘플.
- 배포하면, 입력한 stack name으로 Lambda Application이 생성되고, 아래와 같은 리소스가 생성됨
  - API Gateway
      - 리소스 : API가 정의됨
  - Lambda Function
      - 트리거 : Api Gateway가 트리거로등록됨
- Api Gateway에 설정된 API를 호출하면, lambda가 실행되고, response를 리턴함
- Cloud Formation 이 생성됨(인프라 생성/관리)
...
```bash
sam$ sam logs -n HelloWorldFunction --stack-name sam-stack-name --tail
```
...
```bash
aws cloudformation delete-stack --stack-name sam-stack-name
```
