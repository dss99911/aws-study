...
Info
- This is processed after /etc/hosts, FoxyProxy is handled.
'''
def main():
    import optparse, sys
...
def parse_domain_name(message):
    result = _parse_domain_labels(message)
...
class DNSProxyHandler(BaseRequestHandler):
    def handle(self):
        ...
        q = req.question
        if is_test:
            print("hostname:", q.name)
        if q.type_ in (DNS_TYPE_A, DNS_TYPE_AAAA) and (q.class_ == DNS_CLASS_IN):
            aws_ip = get_aws_ip(q.name)
            if aws_ip:
                packed_ip = addr_p2n(aws_ip)
                self._send_ip_directly(sock, reqdata, q, packed_ip)
                return
    ...
    def _send_ip_directly(self, sock, reqdata, q, packed_ip):
        # header, qd=1, an=1, ns=0, ar=0
        print("reqdata:", reqdata[:2])
        ...
        rspdata += b'\x00' + bytes(chr(len(packed_ip)), 'utf-8') # rd_len
        rspdata += packed_ip
        sock.sendto(rspdata, self.client_address)


def get_aws_ip(hostname):
    if hostname.startswith("ip-") and hostname.endswith('ap-south-1.compute.internal'):
        s = hostname.split(".")[0].split('-') # ex) ip-10-50-0-0.ap-south-1.compute.internal
        if len(s) == 5:
            return ".".join(s[1:])
...
def load_hosts(hosts_file):
    ...
    'hosts file is handled before this.'
